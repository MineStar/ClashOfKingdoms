...
public class cmdRefreshLeader
{
	private static String Description = "Call a new teamleader";
	private static String Syntax = "/cok newLeader red|blu";
	private static String[] Examples = {"/cok newLeader blu"};
	...
	public static void ExecuteCommand(Player player, String[] args)
	{
		...
		if(game.getLeader(TeamID) != null)
		{
			Player oldLeader = CoKCore.getPlayer(game.getLeader(TeamID));
			game.getLeaderClass().clearKit(oldLeader);
		}
		
		game.callNewLeader(TeamID);		
		return;		
	}
